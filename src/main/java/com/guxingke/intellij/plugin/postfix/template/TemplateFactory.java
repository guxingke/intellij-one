...
public class TemplateFactory {
  ...
  public static Set<PostfixTemplate> createTemplates(PostfixTemplateProvider provider) {
    ...
    try {
      var cfgs = Files.list(Paths.get(dir))
  ...
  private static Condition<PsiElement> build(List<TemplateItem> tpls) {
    return expr -> {
      ...
      var ctx = new ConditionContext(primitive, array, generic, normal, clazz, psiClass, componentClazz, cc, e, project, facade);
  ...
  public static TemplateItem match(
    ...
    for (TemplateItem tpl : tpls) {
      // check type
      if (tpl.getType().equals("primitive")) {
        if (!ctx.isPrimitive()) {
          return null;
        }
      ...
      } else if (tpl.getType().equals("generic")) {
        if (!ctx.isGeneric()) {
          return null;
        }
      ...
      } else if (tpl.getType().equals("array")) {
        if (!ctx.isArray()) {
          return null;
        }
        if (!ctx.getClazzName().equals(tpl.getClazz())) {
      ...
      } else if (tpl.getType().equals("normal")) {
        if (!ctx.isNormal()) {
          return null;
        }
        if (!PsiExpressionUtils.isClass(ctx.getExpression(), tpl.getClazz())) {
